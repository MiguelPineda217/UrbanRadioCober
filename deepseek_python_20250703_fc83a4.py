...
class UrbanRadioCober:
    ...
    def intersect(self,layer_predios,buffer,num_id):
        ...
        for feature in resultado_final.getFeatures():
            ...
            value = QgsExpression('$area').evaluate(context)
    ...
    def join(self,layer_predios,layer_buff,num_id):
        ...
        joinObject.setJoinLayerId(layer_buff.id())
        # join field name of layerbuff
        joinObject.setJoinFieldName("gid")
        # join field name of layer_predios
        joinObject.setTargetFieldName("gid")
        joinObject.setUsingMemoryCache(True)
        joinObject.setJoinLayer(layer_buff)
        layer_predios.addJoin(joinObject)
        ...
        for feature in layer_predios.getFeatures():
            context.setFeature(feature)
            value = QgsExpression('"'+layer_buff.name()+'_area_buff'+str(num_id)+'"').evaluate(context)
            feature["area_buff"+str(num_id)] = value
    ...
    def exporta_capa(self,capa):
        QgsVectorFileWriter.writeAsVectorFormat(capa,"D:/multibuffer/ly_"+capa.name()+".shp", "utf-8", capa.crs(), "ESRI Shapefile")
