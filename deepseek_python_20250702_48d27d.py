...
from qgis.core import (
    ...
    QgsProcessingParameterNumber, QgsProcessingParameterEnum, QgsProcessingOutputVectorLayer
)
...
class UrbanRadioCober:
    ...
    def add_action(
        ...
        parent: Optional[QObject] = None) -> Optional[QAction]:
        ...
        try:
            ...
            self.show_exception(e, "Error creating action")
            return None
    ...
    def initGui(self):
        ...
        try:
            ...
            self.provider = UrbanRadioCoberProvider()
            QgsApplication.processingRegistry().addProvider(self.provider)

        except Exception as e:
            self.show_exception(e, "Error initializing GUI")
    ...
    def unload(self):
        ...
        try:
            ...
            self.show_exception(e, "Error unloading plugin")
        finally:
            uninstallErrorHook()
    ...
    def run(self):
        ...
        try:
            ...
            # Use processing context for better progress reporting
            with OverrideCursor(Qt.WaitCursor):
                ...
                params = {
                    'INPUT_LAYER': active_vl,
                    'POWER': float(self.dlg.ui.txtcodU_6.text()),
                    'FREQUENCY': float(self.dlg.ui.txtcodU_5.text()),
                    'TRANSMITTER_GAIN': float(self.dlg.ui.txtAreaU_8.text()),
                    'LOSSES': float(self.dlg.ui.txtNomU_4.text()),
                    'SENSITIVITY': float(self.dlg.ui.txtNomU_7.text()),
                    'OBSTACLES': float(self.dlg.ui.txtAreaU_7.text()),
                    'RECEIVER_GAIN': float(self.dlg.ui.txtAreaU_9.text()),
                    'AREA_TYPE': self.dlg.ui.comboBox_5.currentText(),
                    'OUTPUT': 'memory:'
                }
                
                # Execute algorithm
                results = processing.run(
                    'urban_radiocober:coverage_simulation',
                    params,
                    context=context,
                    feedback=feedback
                )
            ...
            self.show_exception(e, "Error in main execution")

    def process_coverage(self, active_vl: QgsVectorLayer) -> bool:
        ...
        try:
            ...
            self.show_exception(e, "Error processing coverage")
            return False

    def post_processing(self, layer_final: QgsVectorLayer, vl: QgsVectorLayer, id_select: int):
        ...
        try:
            ...
            # Load styles
            estilo_path = os.path.join(os.path.dirname(__file__), "estilo.qml")
            estilo_buff_path = os.path.join(os.path.dirname(__file__), "estilo_buff.qml")
            ...
            self.show_exception(e, "Error in post-processing")

    def show_warning(self, message: str):
        """Show a warning message"""
        QMessageBox.warning(self.iface.mainWindow(), "Warning", message)

    def show_exception(self, exception: Exception, context: str = None):
        """Show exception details"""
        exc_type, exc_value, exc_traceback = sys.exc_info()
        msg = context or self.tr("An error occurred while executing the plugin")
        showException(
        ...
        QgsMessageLog.logMessage(
            f"Error in UrbanRadioCober plugin: {str(exception)}\n{traceback.format_exc()}",
            "UrbanRadioCober",
            Qgis.Critical
        )
    ...
    def inicia_valores(self):
        ...
        try:
            ...
            self.show_exception(e, "Error initializing values")
    ...
    def radiomaximobuffer(self, num: int) -> float:
        ...
        try:
            ...
            self.show_exception(e, "Error calculating buffer radius")
            return 0.0

    def splitbuff(self, layer_buff: QgsVectorLayer, num_id: int) -> Optional[QgsVectorLayer]:
        ...
        try:
            ...
            self.show_exception(e, f"Error splitting buffer {num_id}")
            return None

    def intersect(self, layer_predios: QgsVectorLayer, buffer: QgsVectorLayer, num_id: int) -> Optional[QgsVectorLayer]:
        """Intersect layers and calculate areas"""
        try:
            output_path = os.path.join(os.path.dirname(__file__), f"temp/predios_buff{num_id}.shp")
            ...
            self.show_exception(e, f"Error intersecting buffer {num_id}")
            return None
    ...
    def creadir(self):
        ...
        try:
            ...
            self.show_exception(e, "Error creating directory")

    def crearlayerfinal(self, layer_predios: QgsVectorLayer) -> Optional[QgsVectorLayer]:
        ...
        try:
            ...
            self.show_exception(e, "Error creating final layer")
            return None

    def join(self, layer_predios: QgsVectorLayer, layer_buff: QgsVectorLayer, num_id: int):
        ...
        try:
            ...
            self.show_exception(e, f"Error joining buffer {num_id}")

    def borra_null(self, layer_final: QgsVectorLayer):
        ...
        try:
            ...
            self.show_exception(e, "Error cleaning null values")

    def asigna_calidad(self, layer_final: QgsVectorLayer, id_selfea: int):
        ...
        try:
            ...
            self.show_exception(e, "Error assigning signal quality")

    def exporta_capa(self, capa: QgsVectorLayer) -> Optional[QgsVectorLayer]:
        """Export layer to shapefile"""
        try:
            output_dir = os.path.join(os.path.dirname(__file__), "temp")
            os.makedirs(output_dir, exist_ok=True)
            
            output_path = os.path.join(output_dir, f"ly_{capa.name()}.shp")
            ...
            self.show_exception(e, f"Error exporting layer {capa.name()}")
            return None

    def carga_simbol(self, layer: QgsVectorLayer, symbol_path: str):
        """Load layer style"""
        try:
            if os.path.exists(symbol_path):
                layer.loadNamedStyle(symbol_path)
        except Exception as e:
            self.show_exception(e, f"Error loading style {symbol_path}")

    def captura_id(self, capa: QgsVectorLayer) -> int:
        """Get selected feature ID"""
        try:
            selected = capa.selectedFeatures()
            return selected[0]["gid"] if selected else -1
        except Exception as e:
            self.show_exception(e, "Error getting feature ID")
            return -1


class UrbanRadioCoberProvider(QgsProcessingProvider):
    """Processing provider for UrbanRadioCober plugin"""

    def __init__(self):
        super().__init__()
        
    def id(self) -> str:
        return 'urban_radiocober'

    def name(self) -> str:
        return self.tr('Urban Radio Coverage')

    def icon(self) -> QIcon:
        return QIcon(':/plugins/UrbanRadioCober/icon.png')

    def loadAlgorithms(self):
        self.addAlgorithm(UrbanRadioCoberAlgorithm())

    def load(self) -> bool:
        return True


class UrbanRadioCoberAlgorithm(QgsProcessingAlgorithm):
    """Processing algorithm for UrbanRadioCober plugin"""
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                'SENSITIVITY',
                self.tr('Minimum sensitivity (dB)'),
                type=QgsProcessingParameterNumber.Double,
                defaultValue=-69.0
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                'AREA_TYPE',
                self.tr('Area type'),
                options=['Residencial', 'Oficinas', 'Comercial/industrial'],
                defaultValue=0
            )
        )
        ...
        self.addOutput(
            QgsProcessingOutputVectorLayer(
                'OUTPUT',
                self.tr('Radio coverage')
            )
        )

    def processAlgorithm(self, parameters, context, feedback):
        # Get parameters
        source = self.parameterAsVectorLayer(parameters, 'INPUT_LAYER', context)
        power = self.parameterAsDouble(parameters, 'POWER', context)
        frequency = self.parameterAsDouble(parameters, 'FREQUENCY', context)
        tx_gain = self.parameterAsDouble(parameters, 'TRANSMITTER_GAIN', context)
        losses = self.parameterAsDouble(parameters, 'LOSSES', context)
        sensitivity = self.parameterAsDouble(parameters, 'SENSITIVITY', context)
        obstacles = self.parameterAsDouble(parameters, 'OBSTACLES', context)
        rx_gain = self.parameterAsDouble(parameters, 'RECEIVER_GAIN', context)
        area_type = self.parameterAsEnum(parameters, 'AREA_TYPE', context)
        
        # Create output layer
        crs = source.crs().authid()
        output_layer = QgsVectorLayer(f"Polygon?crs={crs}", "Radio Coverage", "memory")
        
        # Here you would implement the actual radio coverage calculation
        # using the same logic as in the main plugin but adapted for Processing
        
        # For now just return the empty layer as output
    ...
    def displayName(self) -> str:
        return self.tr('Radio Coverage Simulation')
    ...
    def shortHelpString(self) -> str:
        return self.tr("Simulates omnidirectional radio coverage in urban environment")