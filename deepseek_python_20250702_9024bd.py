...
class UrbanRadioCober:
    ...
    def run(self):
        ...
        if result == 1:
            ...
            QgsProject.instance().removeMapLayers([pbuff3.id(), pbuff2.id(), pbuff1.id(), vl.id()])
    ...
    def splitbuff(self, layer_buff, num_id):
        ...
        layer_lotes_anillo1_pr.addAttributes(
            [QgsField("FID", QVariant.Int), QgsField("distance", QVariant.Double, "", 10, 3)])
    ...
    def intersect(self, layer_predios, buffer, num_id):
        # Use processing.run() instead of QgsOverlayAnalyzer
        params = {
            'INPUT': layer_predios,
            'OVERLAY': buffer,
            'OUTPUT': "D:/multibuffer/predios_buff" + str(num_id) + ".shp"
        }
        processing.run("native:intersection", params)
        
        resultado_final = QgsVectorLayer("D:/multibuffer/predios_buff" + str(num_id) + ".shp",
                                       "predios_buff" + str(num_id), "ogr")

        resultado_final.startEditing()

        resultado_final.addAttribute(QgsField("area_buff" + str(num_id), QVariant.Double, "", 10, 3))
        # recalculates the area field
    ...
    def crearlayerfinal(self, layer_predios):
        ...
        context = QgsExpressionContext()
        context.appendScopes(QgsExpressionContextUtils.globalProjectLayerScopes(resultlayer))
        expression = QgsExpression('$area')
        expression.prepare(context)
    ...
    def join(self, layer_predios, layer_buff, num_id):
        ...
        join_info.setPrefix(layer_buff.name() + "_")
    ...
    def borra_null(self, layer_final):
        ...
        for feature in layer_final.getFeatures():
            if feature["area_buff0"] is None:
                feature["area_buff0"] = 0
            if feature["area_buff1"] is None:
                feature["area_buff1"] = 0
            if feature["area_buff2"] is None:
    ...
    def asigna_calidad(self, layer_final, id_selfea):
        ...
        for feature in layer_final.getFeatures():
            if feature["area_buff0"] > feature["area_buff1"] and feature["area_buff0"] > feature["area_buff2"]:
                ...
                feature["Signal"] = "Low"
            elif feature["area_buff2"] is None and feature["area_buff0"] is None and feature["area_buff1"] is None:
                feature["Signal"] = "No signal"
            elif feature["Signal"] is None:
                feature["Signal"] = "No signal"
    ...
    def exporta_capa(self, capa):
        QgsVectorFileWriter.writeAsVectorFormat(capa, "D:/multibuffer/ly_" + capa.name() + ".shp", 
                                              "utf-8", capa.crs(), "ESRI Shapefile")
        capa_shape = QgsVectorLayer("D:/multibuffer/ly_" + capa.name() + ".shp", capa.name(), "ogr")
        QgsProject.instance().addMapLayer(capa_shape)
        capa.commitChanges()
        return capa_shape
